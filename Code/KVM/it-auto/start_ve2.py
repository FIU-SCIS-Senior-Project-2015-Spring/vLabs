...
if needs_provision(): #first_time == 1:
    provision_cmd = "/lclhome/dgonz023/it-auto/provisioner2.py %s %s " % (ve_type, storage_id)
    print provision_cmd
...
if True: 
    try:
        ...
        if(os.path.exists(restore_router)):
            restore_cmd = "virsh restore %s" % (restore_router,)
            print "Command: " + restore_cmd
            subprocess.call([restore_cmd], shell=True)
        else:
            register_cmd = "virt-install --connect qemu:///system --ram %d --name %s --os-type=linux --network network:default --network network:default --network network:default --network network:default --disk path=%s,device=disk,format=qcow2 --vcpus=%d --vnc --force --accelerate --noautoconsole --import" % (XP_PARAMS["memory"], router_name, router_qcow2, XP_PARAMS["ncpus"])
            cmd = register_cmd
            print "Command: " + cmd
        ...
        print "() Call sudo /lclhome/dgonz023/it-auto/vmnat.sh add %d %s %d %d" % (first_port, router_ip, RPORT_START, num_ports)
...
#            f.close()
            start_cmd = "/lclhome/dgonz023/it-auto/start_vm2.py %s %s %s %d %d %s %s" %(snap_file, unique_instance_name, "windows", XP_PARAMS["memory"], XP_PARAMS["ncpus"], router_name, mac)
            print start_cmd
...
#                f.write(dcvm.get_vmx())
                ...
                # f.write("\ncheckpoint.vmState.readOnly = TRUE")
#                f.close()
                start_cmd = "/lclhome/dgonz023/it-auto/start_vm2.py %s %s %s %d %d %s %s %s " %(snap_file, unique_instance_name, "windows", DC_PARAMS["memory"], DC_PARAMS["ncpus"], router_name, mac, mac2)
                print start_cmd
                subprocess.call([start_cmd], shell=True)
                print "============="
#                print dcvm.get_vmx()
