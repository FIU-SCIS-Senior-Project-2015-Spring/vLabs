...
$wsdl=WSDL_VL;//"http://ita-provisioner.cis.fiu.edu:8080/axis2/services/VirtualLabs?wsdl";
$location =LOCATION_VL;//"http://ita-provisioner.cis.fiu.edu:8080/axis2/services/VirtualLabs";
...
}else if ($action=='extendAppointment'){
	header('Content-Type: text/xml');
...
function setWSUserDefaultTimeZone($requestingUser, $usernew){
	...
	//$zone = get_record('user_info_data','userid',$usernew->id,'field_id',4);
...
function isValidAminUser($username){
	...
	//return array(true);
	if($role[0]['user_type'] == "administrator"){
		$result = true;
	}else{
		$result = false;
	}
...
function isValidMentorUser($username){
	//TN will look into what mentor should be
	//$user = get_record('user','username',$username);
...
function getUserTimeZone($userEmail){
   	try {  		
   		$sql = "SELECT data FROM module_vlabs_user_info_data WHERE email = '".$userEmail."' and field_id = 4";
	    //$zone = get_record_sql($sql);
		$zone = eF_ExecuteQuery($sql);
        return $zone;
    } catch (Exception $e) {
        echo $e->getMessage();
    }	
...
function setUserTimeZone($userId, $timeZoneId){
   	try {
   		...
   		//$sql = "UPDATE mdl_user_info_data SET data ='".$timeZoneId."' WHERE userid = ".$userId." and field_id = 4";
		$sql = "UPDATE module_vlabs_user_info_data SET data ='".$timeZoneId."' WHERE userid = ".$userId." and field_id = 4";
...
function setUserFilterOptions($userId, $options){
   	try {
		...
		//if(!count_records_sql(sql)){
			//$sql = "UPDATE mdl_user_info_data SET data ='".$options."' WHERE userid = ".$userId." and field_id = 10";
			$sql = "UPDATE module_vlabs_user_info_data SET data ='".$options."' WHERE userid = ".$userId." and field_id = 10";
			//execute_sql($sql,false);
			eF_ExecuteQuery($sql);
			//echo $sql;
		}else{
			//$sql = "INSERT INTO mdl_user_info_data (userid, field_id, data) VALUES (".$userId.", 10, '".$options."')";
			$sql = "INSERT INTO module_vlabs_user_info_data (userid, field_id, data) VALUES (".$userId.", 10, '".$options."')";
			//execute_sql($sql,false);
			eF_ExecuteQuery($sql);
			//echo $sql;
		}*/
...
function setUserView($userEmail, $view){
   	try {
		...
		//if(!count_records_sql(sql)){
			//$sql = "UPDATE mdl_user_info_data SET data ='".$view."' WHERE userid = ".$userId." and field_id = 12";
			$sql = "UPDATE module_vlabs_user_info_data SET data ='".$view."' WHERE userid = ".$userId." and field_id = 12";
			//execute_sql($sql,false);
			eF_ExecuteQuery($sql);
			//echo $sql;
		}else{
			//$sql = "INSERT INTO mdl_user_info_data (userid, field_id, data) VALUES (".$userId.", 12, '".$view."')";
			$sql = "INSERT INTO module_vlabs_user_info_data (userid, field_id, data) VALUES (".$userId.", 12, '".$view."')";
			//execute_sql($sql,false);
			eF_ExecuteQuery($sql);
			//echo $sql;
		}*/
...
/*
 function getBpp($userId){
    try {  		
   	$sql = "SELECT data FROM module_vlabs_user_info_data WHERE email = '".$userEmail."' and field_id = 19";
...
function getBppValue($userId){
    $bpp =  get_field('user_info_data','data','userid',$userId,'field_id',19);
    return $bpp;
}

function setBpp($userId, $bpp){
   	try {
		if(record_exists('user_info_data', 'userid', $userId, 'field_id', 19)){
			$sql = "UPDATE mdl_user_info_data SET data ='".$bpp."' WHERE userid = ".$userId." and field_id = 19";
			execute_sql($sql,false);
		}else{
			$sql = "INSERT INTO mdl_user_info_data (userid, field_id, data) VALUES (".$userId.", 19, '".$bpp."')";
			execute_sql($sql,false);
		}
		echo "changed";
    } catch (Exception $e) {
...
function getResolution($userId){
    try {  		
   	$sql = "SELECT data FROM module_vlabs_user_info_data WHERE email = '".$userEmail."' and field_id = 20";
...
function getResolutionValue($userId){
    $resolution =  get_field('user_info_data','data','userid',$userId,'field_id',20);
    return $resolution;
}
function setResolution($userId, $resolution){
   	try {
		if(record_exists('user_info_data', 'userid', $userId, 'field_id', 20)){
			$sql = "UPDATE mdl_user_info_data SET data ='".$resolution."' WHERE userid = ".$userId." and field_id = 20";
			execute_sql($sql,false);
		}else{
			$sql = "INSERT INTO mdl_user_info_data (userid, field_id, data) VALUES (".$userId.", 20, '".$resolution."')";
			execute_sql($sql,false);
		}
		echo "changed";
    } catch (Exception $e) {
