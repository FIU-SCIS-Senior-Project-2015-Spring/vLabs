...
var defaults = {
	...
	// display
	defaultView: 'agendaList',	// month
	aspectRatio: 1.35,
	header: {
		left: 'title',
		center: '',
		right: 'gCal newAppt today prev,next'	//// Edited: JAM
	},
	weekends: true,
	...
	titleFormat: {
		...
		day: 'dddd, MMM d, yyyy',
		list: 'MMMM yyyy'	//// Added: JAM
	},
	columnFormat: {
		month: 'ddd',
		week: 'ddd M/d',
		day: 'dddd M/d',
		list: "d{ 'to' d}"	//// Added: JAM
	},
	timeFormat: { // for event elements
		// Modified by SMS: 8/8/2011
		month: "HH:mm-{HH:mm}",		//// Added: JAM
		// month: "h(:mm)t{ '-' h(:mm)t}",		//// Added: JAM
		list: "MMMM d, yyyy h(:mm)tt{ '-' MMMM d, yyyy h(:mm)tt}",	//// Added: JAM
		'': 'h(:mm)t' // default
	},
	displayFormat: {
		'': "ddd, MMMM dS, yyyy h(:mm)tt{ '-' ddd, MMMM dS, yyyy h(:mm)tt}"
		//'': "ddd, MMMM dS, yyyy, hh:MM:ss TT"
	},
...
var rtlDefaults = {
	header: {
		left: 'next,prev today newAppt gCal',	//// Added: JAM
		center: '',
		right: 'title'
	},
	buttonText: {
...
$.fn.fullCalendar = function(options) {
	...
	this.each(function() {
		...
		var publicMethods = {
			...
			newAppt: function() {		// Added: Jam 06/30/2010
	
				return view.trigger('createEventClick');
			},
			
			gCal: function() {		// Added: Jam 06/30/2010
	
				return view.trigger('showGoogleCal');
			},
			...
			newEvent: function(event) { // create a new event - //// Added: JAM
			...
			loading: function(view, status) {		//// Added: JAM 06/30/2010
	
				view.trigger('loading', view, status);
			},
			/*
			afterListRender: function() {
				
				view.trigger('afterListRender');
...
function Grid(element, options, methods) {
	...
	function renderGrid(r, c, colFormat, showNumbers) {
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			tbody.find('td').dblclick(dayDblClick);	//// Added: JAM
			...
			tbody.find('td.fc-new').dblclick(dayDblClick);	//// Added: JAM
	...
	function dayDblClick(ev) {
		...
		view.trigger('dayDblClick', this, date, true, ev);
	}
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		...
		html +=
			"<div id='"+view.name+"-event-" + event.id + "' class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px'>" +
				"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
					(!event.allDay && seg.isStart ?
						"<span class='fc-event-time'>" +
							// Modified by SMS: 8/8/2011
							htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'), options) + ' ' + event.title + ' ' + event.username) + 
	...
	view.trigger('afterMonthRender');
	
}

//// Added: JAM - sets up the list items for the accordion view (List)
function _renderListSegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	var dateformatter = "ddd, MMMM dS, yyyy, hh:MM:ss TT (Z)";
	
	// Orders the eventlistings.
	//segs.sort(function(a,b){return a.event.start.getDate() - b.event.start.getDate();});
	segs.sort(function(a,b){return a.event.start - b.event.start});
	...
	for (i=0; i<segCnt; i++) {
		...
		var id = event.id;
		var start = (event.start) ? event.start : "";
		var end = (event.end) ? event.end : "";
		var allday = event.allDay;
		
			
		if(view.start.getMonth() == start.getMonth()){
			
			listitem += '<div id="'+view.name+'-event-'+ id +'" class="' + className + event.className.join(' ') + '-accordion">';
			// Modifed by SMS: 8/8/2011
			listitem += '<h3 id="fc-'+view.name+'-event-header-'+id+'"><a href="#">' +
						htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'), options) + ' ' + title + ' ' + event.username) +'</a></h3>';
			// listitem += '<h3 id="fc-'+view.name+'-event-header-'+id+'"><a href="#">' + htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'), options)) + ' - ' + title + +'</a></h3>';
			listitem += '<div class="fc-'+view.name+'-event" id="fc-'+view.name+'-event-content-'+id+'">';
			
			listitem += getEventInfo(event,view);
			listitem += getEventActions(event,view);
			//listitem += '<span class="actions-cancel" id="fc-'+view.name+'-event-cancel-'+id+'">&nbsp;</span>';
			//listitem += '<span class="actions-edit" id="fc-'+view.name+'-event-edit-'+id+'">&nbsp;</span>';
			
			listitem += '<p class="time"><b>From:&nbsp;</b>' +
						 htmlEscape(formatDates(event.start,null, view.option('displayFormat'), options)) +'</p>';
			listitem += '<p class="time"><b>To:&nbsp;</b>' +
						htmlEscape(formatDates(event.end,null, view.option('displayFormat'), options)) +'</p>';
			listitem += '<p class="time"><b>Course:&nbsp;</b>'+ event.course +'</p>';
			//listitem += '<p class="time"><b>Id:&nbsp;</b>'+ event.id.toString() +'</p>';
			//listitem += '<p class="time"><b>AllDay Event:&nbsp;</b>'+ allday.toString() +'</p>';
			listitem += '</div>';
			listitem += "</div>";
	...
	//segmentContainer[0].innerHTML = html; // faster than html()
	segmentContainer[0].innerHTML = listitem;
	eventElements = segmentContainer.children();
	...
	for (i=0; i<segCnt; i++) {
		...
		if (triggerRes === false) {
			...
			*/
			...
			/// Added: to check if the event is editable for the list view
			if(!is_mentor_user){
				if(event.type == "scheduled"){
					event.editable = (isEditableDate(event.start)) ? true: false;
					//alert("is_other_user - scheduled: " + event.editable);
				}else{
					event.editable = false;
					//alert("is_other_user - available" + event.editable);
				}
			}else{
				if(event.type == "available"){
					event.editable = (isEditableDate(event.start)) ? true: false;
					//alert("is_mentor_user - available" + event.editable);
				}else{
					event.editable = false;
					//alert("is_mentor_user - scheduled" + event.editable);
				}
			}
			
			//view.trigger('eventListRender');				/// Added: counter for appointments Total
			bindSegHandlers(event, eventElement);			/// Added: assign eventClicks for appointment buttons
			view.reportEventElement(event, eventElement);
		}
	}
	...
	*/
	...
	//renderedEvents = eventElements.length;		// Fixes a bug going from List view to another view.
	
	//alert("afterListRender len: "+eventElements.length);
...
/* NEW Agenda Views: agendaList
-----------------------------------------------------------------------------*/

//// Added: JAM
views.agendaList = function(element, options) {
	return new ListGrid(element, options, {
		render: function(date, delta) {
			if (delta) {
				addMonths(date, delta);
				date.setDate(1);
...
function ListGrid(element, options, methods) {
	...
	function renderGrid(r, c, colFormat, showNumbers) {
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			var endDate = new Date(d);	///// Added: JAM
			endDate.setDate(daysInMonth(d.getMonth()+1, d.getYear()));	///// Added: JAM
		
			var table = $("<table/>").appendTo(element);
			
			s = "<thead><tr>";
			for (i=0; i<colCnt; i++) {
				s += "<th id='listheader' class='fc-" +
					dayIDs[d.getDay()] + ' ' + // needs to be first
					tm + '-state-default' +
					(i==dit ? ' fc-leftmost' : '') +
					"'>" + formatDates(view.visStart, endDate, colFormat, options) + "</th>";	//// Edited: JAM
				addDays(d, 1);
			...
			s += "<tr class='fc-"+view.name+"-row'><td class='fc-"+view.name+"-col'>";
			s += "<div class='fc-"+view.name+"-content'></div>";
			s += "</td></tr>";
			
			
			tbody = $(s + "</tbody>").appendTo(table);
			//tbody.find('td').click(dayClick);
			tbody.find('td.fc-'+view.name+'-col').dblclick(dayDblClick);	// Added: Jam
			
			segmentContainer = $("<div id='accordion' style='border: 1px solid #CCCCCC; min-height: 500px;'/>").appendTo(".fc-"+view.name+"-content");		//element
			...
			//Edited: JAM - to allow the column header date of the ListView to change accordingly. (in this view ther will never be 1 row.)
			//if (rowCnt == 1) { // more changes likely (week or day view)
			...
			endDate.setDate(daysInMonth(d.getMonth()+1, d.getYear()));	///// Added: JAM
				...
				thead.find('th').each(function() {
					$(this).text(formatDates(d, endDate, colFormat, options));
					this.className = this.className.replace(/^fc-\w+(?= )/, 'fc-' + dayIDs[d.getDay()]);
					addDays(d, 1);
					if (nwe) {
						skipWeekend(d);
	...
	*/
	...
	function dayDblClick(ev) {
		...
		if(view.visStart > date){
			view.trigger('dayDblClick', this, view.visStart, true, ev);
		}else{
			view.trigger('dayDblClick', this, date, true, ev);
		}
	}
	...
	function parseMonthStartEvents(events){
		var newEvents = [];
		j = 0;
		
		for(var i=0; i<events.length; i++){
		
			if(events[i].start >= view.start){
	...
	function compileSegs(events) {		// Edited: JAM
	 	var _events = parseMonthStartEvents(events);
		var d1 = cloneDate(view.visStart);
		var dayCnt = daysInMonth(d1.getMonth()+1, d1.getYear())-1;
		var d2 = addDays(cloneDate(d1), dayCnt + 1),
			visEventsEnds = $.map(_events, visEventEnd),
			i, row,
			j, level,
			k, seg,
			segs=[];
		...
		for (i=0; i<rowCnt; i++) {
			row = stackSegs(view.sliceSegs(_events, visEventsEnds, d1, d2));
			for (j=0; j<row.length; j++) {
				level = row[j];
				for (k=0; k<level.length; k++) {
					seg = level[k];
	...
	function bindSegHandlers(event, eventElement, seg) {
		...
		var editButton = $(eventElement).find(".actions-edit");
		var cancelButton = $(eventElement).find(".actions-cancel");
		var confirmButton = $(eventElement).find(".actions-confirm");
		var infoButton = $(eventElement).find(".actions-info");
		
		view.eventCancelButtonHandler(event,cancelButton);
		view.eventEditButtonHandler(event, editButton);
		view.eventConfirmButtonHandler(event, confirmButton);
		view.eventInfoButtonHandler(event, infoButton);
									   

		//alert("bindSegHandlers: "+editBtn.id);
		/*
		if (event.editable || event.editable == undefined && options.editable) {
			draggableEvent(event, eventElement);
			if (seg.isEnd) {
...
function Agenda(element, options, methods) {
	...
	function renderAgenda(c, colFormat) {
		...
		if (!head) { // first time rendering, build from scratch
			...
			head.find('td').dblclick(slotDblClick);		// Added: Jam
			...
			body.find('td').dblclick(slotDblClick);		// Added: Jam
	...
	// Added: Dbl Click day cell in table
	function slotDblClick(ev) {
		...
		if (rowMatch) {
			...
			date.setHours(hours);
			date.setMinutes(mins%60 + minMinute);
			view.trigger('dayDblClick', this, date, false, ev);
		}else{
			view.trigger('dayDblClick', this, date, true, ev);
		}
	}
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			if (levelI >= 0) {	//if (levelI) {	// JAM: edited 09.15.2011 - because the level is 0, the first always is wider
				// indented and thin
			...
			html +=
				"<div id='"+view.name+"-event-"+event.id+"' class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + top + "px;left:" + left + "px'>" +
					"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
						"<span class='fc-event-bg'>&nbsp;</span>" +
						// Modified by SMS: 8/8/2011
						"<span class='fc-event-time actions'>" + htmlEscape(formatDates(event.start, event.end, view.option('timeFormat')) + ' ' + event.title + ' ' + event.username) + "</span>" +
						// "<span class='fc-event-time actions'>" + htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'))) + "</span>" +
						"<span class='fc-event-actions'>" + getEventActions(event,view) + "</span>" + 
		...
		for (i=0; i<segCnt; i++) {
			...
			if (eventElement = seg.element) {
				//eventElement[0].style.width = seg.outerWidth - seg.hsides + 'px';
				eventElement[0].style.width = ((seg.outerWidth - seg.hsides) + 6) + 'px';	// Added 6 px for icons span
				...
				if (seg.titleTop != undefined && height - seg.titleTop < 10) {
					...
					eventElement.find('span.fc-event-actions').remove();
					eventElement.find('span.fc-event-time').removeClass('actions');
				}
				view.trigger('eventAfterRender', event, event, eventElement);
			}
		}
...
/* Added: for eventActions   JAM*/
function getEventActions(event,view){
	var actions = event.actions;
	var listitem = "";
	var infoSpan = "";
	if($.isArray(actions)){
		for(i in actionNames){
			if(actionNames[i]!="info"){	// Place the info span in a different spot.
				for(var a=0; a<actions.length; a++){
					
					if(actions[a].name == actionNames[i]){
						//var actionId = "fc-"+actions[a].name+"-"+event.id;
						var actionId = "fc-"+view.name+"-event-"+actions[a].name+"-"+event.id;
						var icon = getActionIcon(actions[a].name);	
						if(view.name == "agendaWeek" || view.name == "agendaDay"){
							listitem += "<span id='"+actionId+"' class='ui-icon ui-icon-"+icon+"' style='float:right;'>&nbsp;</span>";
						}else if(view.name == "agendaList"){
							listitem += '<span id="'+actionId+'" class="actions-'+actions[a].name+'">&nbsp;</span>';
...
function getEventInfo(event,view){
	var actions = event.actions;
	var infoSpan = "";
	if($.isArray(actions)){
		for(i in actionNames){
			if(actionNames[i]=="info"){	// Place the info span in a different spot.
				for(var a=0; a<actions.length; a++){
					...
					if(actions[a].name == actionNames[i]){
						...
						if(view.name == "agendaWeek" || view.name == "agendaDay"){
							infoSpan = "<span class='fc-event-info'>";
							infoSpan += "<span id='"+actionId+"' class='ui-icon ui-icon-"+icon+"'></span>";
							infoSpan += "</span>";
						}else if(view.name == "agendaList"){
							infoSpan += '<span id="'+actionId+'" class="actions-'+actions[a].name+'">&nbsp;</span>';
						}else if (view.name == "month"){
							// NOT USED
						}
					}
...
var viewMethods = {
	...
	eventElementHandlers: function(event, eventElement) {
		...
		eventElement
				...
				setTimeout(function() {
					...
					if(!isdblclick){
					   ...
					   //if(view.name != "agendaList"){
							...
							//$("#debug").text("eventElementHandlers: click");
						//}
					}
				}, 500)
			...
			.mousedown(function(ev) {
				//$("#debug").text("eventElementHandlers: mousedown");
			
			})
			.mouseup(function(ev) {
				//$("#debug").text("eventElementHandlers: mouseup");
	...
	// attaches eventClick, eventMouseover, eventMouseout for Edit and Cancel Button Clicks
	
	eventEditButtonHandler: function(event, eventElement) {
		var view = this;
		eventElement
			.click(function(ev) {
				return view.trigger('eventEditClick', this, event, ev);
			});
	},
	
	eventCancelButtonHandler: function(event, eventElement) {
		var view = this;
		eventElement
			.click(function(ev) {
				return view.trigger('eventCancelClick', this, event, ev);
			});
	},
	
	eventConfirmButtonHandler: function(event, eventElement) {
		var view = this;
		eventElement
			.click(function(ev) {
				return view.trigger('eventConfirmClick', this, event, ev);
			});
	},
	
	eventInfoButtonHandler: function(event, eventElement) {
		var view = this;
		eventElement
			.click(function(ev) {
				return view.trigger('eventInfoClick', this, event, ev);
			});
	},
		...
		for (i=0; i<len; i++) {
			...
			if (eventEnd > start && eventStart < end) {
				...
				if (eventStart < start) {
					...
					isStart = false;
					//alert(event.id+' debug: 1 \n segStart: '+segStart);
				}else{
					segStart = eventStart;
					isStart = true;
					//alert(event.id+' debug: 2 \n segStart: '+segStart);
				}
				if (eventEnd > end) {
					//if (eventEnd.toString() == end.toString()) {
					//	segEnd = eventEnd;
					//	isEnd = true;
					//}else{
						...
						isEnd = false;
					//}
					//alert(event.id+' debug: 3 \n segEnd: '+segEnd);
				}else{
					segEnd = eventEnd;
					isEnd = true;
					//alert(event.id+' debug: 4 \n segEnd: '+segEnd);
				}
				
				if(segStart.toString() == segEnd.toString()){
...
function lazySegBind(container, segs, bindHandlers) {
	container.unbind('mouseover').mouseover(function(ev) {
		...
		if(e){ // Added: Jam - e is indefined in list view
